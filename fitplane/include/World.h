...
#include "tf2_geometry_msgs/tf2_geometry_msgs.hpp"  // 用于转换类型
...
public:
    ...
    World(const float &resolution, const rclcpp::Node::SharedPtr nh); //, FitPlaneArg fitarg)
    ~World();
    ...
    void GetRobotPosition()
    {
        ...
        {
            // 获取变换
            transform_pose = buffer_->lookupTransform("map", "base_link", rclcpp::Time(0));
