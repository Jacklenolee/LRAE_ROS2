...
public:
  ...
  void loadRosParamFromNodeHandle(const rclcpp::Node::SharedPtr nh)
  {
    ...
    nh->declare_parameter<double>("xy_goal_tolerance", xy_goal_tolerance);
    nh->declare_parameter<double>("transform_tolerance", transform_tolerance);
    nh->declare_parameter<double>("max_global_plan_lookahead_dist", max_global_plan_lookahead_dist);

    nh->get_parameter("odom_topic", odom_topic);
    nh->get_parameter("xy_goal_tolerance", xy_goal_tolerance);
    nh->get_parameter("transform_tolerance", transform_tolerance);
    nh->get_parameter("max_global_plan_lookahead_dist", max_global_plan_lookahead_dist);
   
    // Robot
    nh->declare_parameter<double>("max_vel_x", max_vel_x);
    nh->declare_parameter<double>("min_vel_x", min_vel_x);
    nh->declare_parameter<double>("max_vel_theta", max_vel_theta);
    nh->declare_parameter<double>("min_vel_theta", min_vel_theta);
    nh->declare_parameter<double>("acc_lim_x", acc_lim_x);
    nh->declare_parameter<double>("acc_lim_theta", acc_lim_theta);
    nh->declare_parameter<double>("control_period", control_period);
    ...
    nh->get_parameter("acc_lim_x", acc_lim_x);
    nh->get_parameter("acc_lim_theta", acc_lim_theta);
    nh->get_parameter("control_period", control_period);

    // Weights
    nh->declare_parameter<double>("path_distance_bias", path_distance_bias);
    nh->declare_parameter<double>("goal_distance_bias", goal_distance_bias);
    nh->declare_parameter<double>("occdist_scale", occdist_scale);

    nh->get_parameter("path_distance_bias", path_distance_bias);
    nh->get_parameter("goal_distance_bias", goal_distance_bias);
    nh->get_parameter("occdist_scale", occdist_scale);
  }
};
class DWAConfig
{
public:
  ...
  void loadRosParamFromNodeHandle(const rclcpp::Node::SharedPtr nh)
  {
    ...
    // DWA
    nh->declare_parameter<int>("sim_time_samples", sim_time_samples);
    nh->declare_parameter<int>("vx_samples", vx_samples);
    nh->declare_parameter<int>("vth_samples", vth_samples);
    nh->declare_parameter<double>("path_distance_bias", path_distance_bias);
    nh->declare_parameter<double>("goal_distance_bias", goal_distance_bias);
    nh->declare_parameter<double>("occdist_scale", occdist_scale);
    nh->declare_parameter<double>("sim_distance", sim_distance);

    nh->get_parameter("sim_time_samples", sim_time_samples);
    nh->get_parameter("vx_samples", vx_samples);
    nh->get_parameter("vth_samples", vth_samples);
    nh->get_parameter("path_distance_bias", path_distance_bias);
    nh->get_parameter("goal_distance_bias", goal_distance_bias);
    nh->get_parameter("occdist_scale", occdist_scale);
    nh->get_parameter("sim_distance", sim_distance);
  }
};
