...
bool ExplorationPlanning::initialize()
{
    ...
    nh_->declare_parameter<double>("lrae_planner_node/angle_pen", angle_pen_);
    nh_->declare_parameter<int>("lrae_planner_node/update_cen_thre", update_cen_thre_);
    nh_->declare_parameter<int>("lrae_planner_node/unknown_num_thre", unknown_num_thre_);
    nh_->declare_parameter<double>("lrae_planner_node/minrange", minrange_);
    nh_->declare_parameter<bool>("lrae_planner_node/limit_max_square", limit_max_square_);
    nh_->declare_parameter<bool>("lrae_planner_node/use_go_end_nearest", use_go_end_nearest_);
    nh_->declare_parameter<double>("lrae_planner_node/end_neacen_disthre", end_neacen_disthre_);
    nh_->declare_parameter<double>("lrae_planner_node/end_cur_disrate", end_cur_disrate_);

    nh_->get_parameter("lrae_planner_node/angle_pen", angle_pen_);
    nh_->get_parameter("lrae_planner_node/update_cen_thre", update_cen_thre_);
    nh_->get_parameter("lrae_planner_node/unknown_num_thre", unknown_num_thre_);
    nh_->get_parameter("lrae_planner_node/minrange", minrange_);
    nh_->get_parameter("lrae_planner_node/limit_max_square", limit_max_square_);
    nh_->get_parameter("lrae_planner_node/use_go_end_nearest", use_go_end_nearest_);
    nh_->get_parameter("lrae_planner_node/end_neacen_disthre", end_neacen_disthre_);
    nh_->get_parameter("lrae_planner_node/end_cur_disrate", end_cur_disrate_);
        ...
        else
        {
            ...
            else
            {
                    ...
                    && (abs(ego_position_.x - last_near_cen_.centroidposition.x) > 10 || abs(ego_position_.y - last_near_cen_.centroidposition.y) > 10))
                {
                    ...
                    exporation_path.header.stamp = clock->now();
